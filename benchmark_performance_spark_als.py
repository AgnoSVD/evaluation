...
from als_pyspark import validation
...
class Benchmark:
    def __init__(self, metric, train_size, density_list, als_params, sgd_params):
        ...
        self.spark = SparkSession \
            .builder \
            .appName("recommendation system 1") \
    ...
    def run_als(self, train_df, test_job_id, known_confs, K, n_epochs):
        ...
        ref_nodes = self.ref_df.drop("timestamp", axis=1).values
        # get spark context
        sc = self.spark.sparkContext
        sc.setLogLevel("ERROR")
        train_data = sc.parallelize([(int(x[0]), int(x[1]), float(x[2])) for x in train_nodes])
        start = time.time()
        model = ALS.train(
            ratings=train_data,  # (userID, productID, rating) tuple
            iterations=10,
            rank=8,
            lambda_=0.001,  # regularization param
            seed=99)
        # model.fit(trainset)
        ...
        validation_data = sc.parallelize((int(x[0]), int(x[1]), float(x[2])) for x in ref_list)
        test_data = validation_data.map(lambda p: (p[0], p[1]))
        predictions = model.predictAll(test_data).map(lambda r: ((r[0], r[1]), r[2]))
        predictions = validation_data.map(lambda r: ((r[0], r[1]), r[2])).join(predictions).collect()
        def apply_objective(x):
            config = self.config_info[int(x[0][1])]

            est_real = self.denormalize(x[1][0])
            r_ui_real = self.denormalize(x[1][1])
            if self.metric == "perf":
                return Prediction(x[0][0], x[0][1], x[1][1], x[1][0], None)
            elif self.metric == "cost":
                ...
                return Prediction(x[0][0], x[0][1], r_ui, est, None)
            else:
